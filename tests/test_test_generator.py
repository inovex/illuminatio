...
def test__generate_test_cases__allow_some_pods__returns_negative_and_positive_case():
    ...
    namespaces = [_generate_namespace(allowed_namespace)]
    networkpolicies = [
        _generate_allow_labelled_pods_network_policy(
            allowed_namespace, labels=allowed_labels
        )
    ]
    expected = [
        NetworkTestCase(
...
def _generate_allow_labelled_pods_network_policy(namespace, labels=None, port=None):
    ...
    policy.spec.ingress = [
        k8s.client.V1NetworkPolicyIngressRule(_from=_from, ports=ports)
    ]
    return policy
