...
class NetworkTestOrchestrator:
    ...
    def refresh_cluster_resources(self, api: k8s.client.CoreV1Api):
        ...
        self.logger.debug(format_string.format(len(namespaces), "namespaces", namespaces))
    ...
    def _rewrite_ports_for_host(self, port_list, services_for_host):
        ...
        for wildcard_port in wild_card_ports:
            ...
            rewritten_ports[wildcard_port] = "%s%s" % (prefix, str(service_ports[0].port))
        ...
        for host_string in target_dict.keys():
            ...
            if isinstance(host, GenericClusterHost):
                self.logger.debug("Found GenericClusterHost %s,"
                                  "Rewriting it to a ClusterHost in default namespace now.", host)
        ...
        while len(result_config_maps) < len(daemon_pods):
            try:
                ...
                if api_exception.reason == "Not Found":
                    ...
                    raise api_exception
            self.logger.debug("Map names: %s", [m.metadata.name for m in result_config_maps])
    ...
    def create_daemonset_manifest(self, daemon_set_name, service_account_name, config_map_name, container_runtime):
        ...
        if container_runtime.startswith("docker"):
            ...
            # We use this old path "/run/containerd/containerd.sock"
