...
class ConcreteClusterHost(Host):
    ...
    def __str__(self):
        return "ConcreteClusterHost(namespace=%s, name=%s)" % (
            str(self.namespace),
            str(self.name),
        )
...
class ClusterHost(Host):
    ...
    def matches(self, obj):
        ...
        if isinstance(obj, k8s.client.V1Pod):
            return (
                ...
                and all(
                    item in obj.metadata.labels.items()
                    for item in self.pod_labels.items()
                )
            )
        if isinstance(obj, k8s.client.V1Service):
            return (
                ...
                and all(
                    item in obj.spec.selector.items()
                    for item in self.pod_labels.items()
                )
            )
    ...
    def __str__(self):
        return "ClusterHost(namespace=%s, podLabels=%s)" % (
            str(self.namespace),
            str(self.pod_labels),
        )
...
class GenericClusterHost(Host):
    ...
    def matches(self, obj):
        if obj is None:
            raise ValueError("obj to match to cannot be None")
        if isinstance(obj, k8s.client.V1Namespace):
            return obj.metadata.labels is not None and all(
                item in obj.metadata.labels.items()
                for item in self.namespace_labels.items()
            )
        ...
        namespace_matches = namespace.metadata.labels is not None and all(
            item in namespace.metadata.labels.items()
            for item in self.namespace_labels.items()
        )
        if isinstance(obj, k8s.client.V1Pod):
            return (
        ...
        return "%s:%s" % (
            (
                ...
                if not self.namespace_labels
                    ...
                    [
                        "%s=%s" % (str(k).strip(), str(v).strip())
                        for k, v in self.namespace_labels.items()
                    ]
                )
            ),
            (
